...
pub fn analyze_instruction_diversity(world: &World) -> HashMap<char, usize> {
    ...
    for org in world.grid.iter().flatten() {
...
pub fn analyze_task_potential(world: &World) -> TaskPotentialStats {
    ...
    for org in world.grid.iter().flatten() {
        ...
        let has_io = org.genome.iter().any(|i| matches!(i, Instruction::IO));
        if has_io {
            stats.organisms_with_io += 1;
        }

        let has_arithmetic = org
            .genome
            .iter()
            .any(|i| matches!(i, Instruction::Add | Instruction::Sub | Instruction::Nand));
        if has_arithmetic {
            stats.organisms_with_arithmetic += 1;
        }

        if has_io && has_arithmetic {
...
impl TaskPotentialStats {
    pub fn print_report(&self) {
        ...
        println!(
            "  Average: {:.2}",
            self.merit_sum / self.total_organisms as f64
        );
