...
impl eframe::App for AvidaApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::left("stats_panel").min_width(250.0).show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Copy:");
                ui.add(egui::DragValue::new(&mut self.world.copy_mutation_rate)
                    .speed(0.0001)
                    .range(0.0..=1.0));
            });
            ...
            ui.horizontal(|ui| {
                ui.label("Insertion:");
                ui.add(egui::DragValue::new(&mut self.world.insertion_rate)
                    .speed(0.001)
                    .range(0.0..=1.0));
            });
            ...
            ui.horizontal(|ui| {
                ui.label("Deletion:");
                ui.add(egui::DragValue::new(&mut self.world.deletion_rate)
                    .speed(0.001)
                    .range(0.0..=1.0));
            });
        ...
        if self.show_inspector {
            egui::SidePanel::right("inspector_panel").min_width(300.0).show(ctx, |ui| {
                ...
                if let Some((x, y)) = self.selected_cell {
                    if let Some(org) = self.world.get_organism(x, y) {
                        ...
                        for (i, chunk) in genome_str.chars().collect::<Vec<_>>().chunks(50).enumerate() {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let cell_size = (available_size.x.min(available_size.y) / width.max(height) as f32).max(2.0);
